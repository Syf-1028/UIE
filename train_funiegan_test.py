...
parser.add_argument("--num_epochs", type=int, default=101, help="number of epochs of training")  # 从201改为10
# 🎯 修改点3: 减小批次大小，降低GPU内存需求
parser.add_argument("--batch_size", type=int, default=16, help="size of the batches")    # 从8改为4
...
if not os.path.exists(args.cfg_file):
    print(f"错误: 找不到配置文件 {args.cfg_file}")
    print("请确保 configs/train_ufo.yaml 文件存在")
    sys.exit(1)
...
samples_dir = os.path.join("samples/FunieGAN201/", dataset_name)
checkpoint_dir = os.path.join("checkpoints/FunieGAN201/", dataset_name)
...
if args.epoch == 0:
    ...
    generator.load_state_dict(torch.load("checkpoints/FunieGAN201/%s/generator_%d.pth" % (dataset_name, args.epoch)))
    discriminator.load_state_dict(torch.load("checkpoints/FunieGAN201/%s/discriminator_%d.pth" % (dataset_name, epoch)))
    print ("🎯 从第 %d 轮加载预训练模型" %(epoch))
...
print(f"🎯 开始训练: 数据集={dataset_name}, 批次大小={batch_size}, 总轮数={num_epochs}")
print(f"🎯 样本保存: {samples_dir}")
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Total loss (Section 3.2.IrdUgan in the paper)
        ...
        if batches_done % val_interval == 0:
            ...
            save_image(img_sample, "samples/FunieGAN201/%s/%s.png" % (dataset_name, batches_done), nrow=5, normalize=True)

    ## Save model checkpoints
    if (epoch % ckpt_interval == 0) or (epoch == num_epochs - 1):
        torch.save(generator.state_dict(), "checkpoints/FunieGAN201/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "checkpoints/FunieGAN201/%s/discriminator_%d.pth" % (dataset_name, epoch))
        print(f"\n🎯 第 {epoch} 轮模型已保存")
