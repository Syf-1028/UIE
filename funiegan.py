...
class UNetDown(nn.Module):
    def __init__(self, in_size, out_size, bn=True):
        ...
        layers = [nn.Conv2d(in_size, out_size, 4, 2, 1, bias=False)]    # 下采样 尺寸减半
        if bn: layers.append(nn.BatchNorm2d(out_size, momentum=0.8))    # 前四层默认增加批量归一化，最后一层不加，避免特征过度平滑
        layers.append(nn.LeakyReLU(0.2))        # 激活函数，缓解梯度消失
...
class UNetUp(nn.Module):
    ...
    def forward(self, x, skip_input):
        x = self.model(x)
        x = torch.cat((x, skip_input), 1)       # 跳跃链接：拼接当前上采样特征与对应下采样层特征（维度1=通道维度）
        return x
...
class DiscriminatorFunieGAN(nn.Module):
    """ A 4-layer Markovian discriminator as described in the paper 马尔可夫判别器
    """
